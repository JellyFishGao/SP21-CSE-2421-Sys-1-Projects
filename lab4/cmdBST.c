...
/* This program takes in the command line arguments to determine: 1.Whether to 
    produce a balance or ordinary BST. 2.Whether to read input from a file and 
    the corresponding file name. After that, ask user for input and construct a 
    blanced or ordinary BST contains characters as nodes. It can add, delete, 
    and print nodes in BST based on user choice. Program quits when user ask for
    quitting. */
...
int main(int argc,char *argv[]) {
    ...
    char fileC; /* char variable to store characters from input file */
    char chaArr[200];/* char array to store the data in unbalanceed BST */
    ...
    FILE *file;/* Possiable file to read */

    begin = clock();
    /* If user entered wrong amount of cmd line arguments, program quit. */
    if(argc == 3 || argc == 4){
        if(argv[2][0] == 'y' && argc == 4){
            /* Need to read a file input */
            file = fopen(argv[3], "r");
            if(file == NULL){
                printf("Error opening the file, program quit.\n");
                return 0;
            }
            while ((fileC = fgetc(file)) != EOF){               
                if(fileC == '\n'){
                    /* Not read the new line in the input file> */
                    continue;
                }else{
                    /* Read character store in ordinary BST */
                    root = add(root, fileC);
                    if(argv[1][0] == 'y'){
                        /* User ask for balanced BST */
    ...
    stop = clock();
    cpu_time_used = ((double) (stop - begin)) / CLOCKS_PER_SEC;
    printf("\nCPU time: %0.20f\n", cpu_time_used);
    return 0;
}
